...
static void requestRadioPower(void *data, size_t datalen, RIL_Token t)
{
	...
	onOff = ((int *)data)[0];
	LOGI("++requestRadioPower %d %d",onOff, sState);
	if (onOff == 0 && sState != RADIO_STATE_OFF) {
		...
		if (err < 0 || p_response->success == 0) {
			/* There was an error, try  AT+CFUN=0*/
			err = at_send_command("AT+CFUN=0", &p_response);
			LOGD("Current Radio State %d %d %d", isRadioOn(), err, p_response->success);
	...
	LOGD("--requestRadioPower--");
	at_response_free(p_response);
	RIL_onRequestComplete(t, RIL_E_SUCCESS, NULL, 0);
	return;

error:
	LOGE("requestRadioPower error");
	at_response_free(p_response);
...
static void requestGetPreferredNetworkType(void *data, size_t datalen, RIL_Token t)
{
	...
	LOGI("requestGetPreferredNetworkType: %d", response);
	RIL_onRequestComplete(t, RIL_E_SUCCESS, &response, sizeof(int));
	at_response_free(p_response);
	return;
...
{
	return "XDandroid/Vogue Community RIL";
}
...
setRadioState(RIL_RadioState newState)
{
	...
	LOGD("setRadioState s_closed %d",s_closed);
	...
	LOGD("setRadioState changing from %d to %d",oldState, newState);
}
...
no_gsm:
	...
	if (phone_has & MODE_CDMA) {
		err = getCDMAOperator();
		if (!err) {
			LOGD("Will fail -> operID %s", operID);
...
static void onUnsolicited (const char *s, const char *sms_pdu)
{
	 ...
	 */
	//LOGI("onUnsolicited state = %d", sState);
	if (sState == RADIO_STATE_UNAVAILABLE) {
		return;
	}
